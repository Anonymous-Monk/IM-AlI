...
@interface SPKitManager ()<YWMessageLifeDelegate,UIAlertViewDelegate>
...
{
    static SPKitManager *sExample = nil;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sExample = [[SPKitManager alloc] init];
    });
...
//            NSString *pwd = [YISTools MD5String:[NSString stringWithFormat:@"XXXX%@",user.id]];
            [self callThisAfterISVAccountLoginSuccessWithYWLoginId:@"visitor511"     passWord:@"taobao1234"   preloginedBlock:^{}
                                                      successBlock:^{
                                                          self.isLogined = YES;
                                                          if (callback) {
                                                              callback();
...
{
    YWPerson *person = [[SPKitManager sharedInstance] exampleFetchEServicePersonWithPersonId:aPersonId groupId:nil];
    [[SPKitManager sharedInstance] exampleOpenConversationViewControllerWithPerson:person fromNavigationController:aNavigationController];
}
...
#warning TODO: JUST RETURN IF NO NEED TO ADD Custom Conversation OR CHANGE TO YOUR ACTUAL METHOD TO ADD Custom Conversation
    ...
    YWCustomConversation *conversation = [YWCustomConversation fetchConversationByConversationId:kSPCustomConversationIdForPortal creatIfNotExist:YES baseContext:[SPKitManager sharedInstance].ywIMKit.IMCore];
    /// 每一次点击都随机的展示未读数和最后消息
...
{
    /// 获取该自定义会话
    YWCustomConversation *conversation = [YWCustomConversation fetchConversationByConversationId:kSPCustomConversationIdForFAQ creatIfNotExist:NO baseContext:[SPKitManager sharedInstance].ywIMKit.IMCore];
    
    if (conversation == nil) {
        /// 还没有则创建
        conversation = [YWCustomConversation fetchConversationByConversationId:kSPCustomConversationIdForFAQ creatIfNotExist:YES baseContext:[SPKitManager sharedInstance].ywIMKit.IMCore];
...
- (void)exampleListenOnClickUrl
{
    ...
    [self.ywIMKit setOpenURLBlock:^(NSString *aURLString, UIViewController *aParentController) {
        ...
        if ([aURLString hasPrefix:@"tel:"]) {
            ...
            YWWebViewController *controller = [YWWebViewController makeControllerWithUrlString:aURLString andImkit:[SPKitManager sharedInstance].ywIMKit];
