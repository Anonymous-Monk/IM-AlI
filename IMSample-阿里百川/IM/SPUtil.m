...
- (id)init
{
    ...
    if (self) {
        ...
        {
            self.controlWaiting = [[UIControl alloc] initWithFrame:[SPKitManager sharedInstance].rootWindow.frame];
...
#warning TODO: CHANGE TO YOUR ACTUAL Profile GETTING METHOD
    ...
    [[[SPKitManager sharedInstance].ywIMKit.IMCore getContactService] asyncGetProfileFromServerForPerson:aPerson withTribe:nil withProgress:^(YWProfileItem *item) {
        if (aProgress) {
...
- (void)syncGetCachedProfileIfExists:(YWPerson *)person completion:(YWFetchProfileCompletionBlock)completionBlock {
    ...
    YWProfileItem *item = [[[SPKitManager sharedInstance].ywIMKit.IMCore getContactService] getProfileForPerson:person withTribe:nil];
...
- (void)setWaitingIndicatorShown:(BOOL)aShown withKey:(NSString *)aKey
{
    ...
    if (self.waitingIndicatorKeys.count > 0) {
        [self.controlWaiting setFrame:[SPKitManager sharedInstance].appDelegate.window.bounds];
        [[SPKitManager sharedInstance].appDelegate.window addSubview:self.controlWaiting];
    } else {
