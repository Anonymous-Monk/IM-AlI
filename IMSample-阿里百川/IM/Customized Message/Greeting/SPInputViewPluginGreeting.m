    ...
    [conversationController.conversation asyncSendMessageBody:body
                                                   ...
                                                   completion:^(NSError *error, NSString *messageID) {
                                                       dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                                                           if (error.code == 0) {
                                                               [[SPKitManager sharedInstance] exampleInsertLocalMessageBody:[[YWMessageBodySystemNotify alloc] initWithContent:@"打招呼成功"] inConversation:weakController.conversation];
                                                           } else {
                                                               [[SPKitManager sharedInstance] exampleInsertLocalMessageBody:[[YWMessageBodySystemNotify alloc] initWithContent:@"打招呼失败"] inConversation:weakController.conversation];
                                                           }
                                                       });
                                                   }];
}
