...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        ...
        if (text.length > 0) {
            ...
            [[SPKitManager sharedInstance] exampleSendTransparentCommand:content inConversation:self.conversationViewController.conversation completion:^(NSError *error, NSString *messageID) {
                if (error.code == 0) {
